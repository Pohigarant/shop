...
@pytest.mark.django_db
def test_update_category_patch(
    request, client_fixture, expected_status, existing_category
):
    client = request.getfixturevalue(client_fixture)
    url = reverse("categories-detail", kwargs={"pk": existing_category.id})
...
def test_search_category(api_client):
    ...
    Category.objects.create(name="Наушники")
    Category.objects.create(name="Ноутбуки")

    url = reverse("categories-list")
    response = api_client.get(url, {"search": "Науш"})

    assert response.status_code == 200
    assert len(response.data["results"]) == 1
    assert response.data["results"][0]["name"] == "Наушники"
