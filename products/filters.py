...
class ProductFilter(filters.FilterSet):
    ...
    name = filters.CharFilter(
        lookup_expr="icontains", label="Название (содержит)"
    )
    model = filters.CharFilter(
        lookup_expr="icontains", label="Модель (содержит)"
    )
    article = filters.CharFilter(
        lookup_expr="icontains", label="Артикул (содержит)"
    )

    # 2. Универсальный поиск по трём полям
    search = filters.CharFilter(
        method="filter_search", label="Поиск (имя/модель/артикул)"
    )
    ...
    quantity_min = filters.NumberFilter(
        field_name="quantity", lookup_expr="gte", label="Количество от"
    )
    quantity_max = filters.NumberFilter(
        field_name="quantity", lookup_expr="lte", label="Количество до"
    )

    # 5. Булевы фильтры
    is_active = filters.BooleanFilter(label="Активен")
    in_stock = filters.BooleanFilter(
        method="filter_in_stock", label="В наличии"
    )
    ...
    category_id = filters.NumberFilter(
        field_name="category__id", label="Категория (ID)"
    )
    ...
    created_after = filters.DateFilter(
        field_name="created_at", lookup_expr="gte", label="Создан после"
    )
    created_before = filters.DateFilter(
        field_name="created_at", lookup_expr="lte", label="Создан до"
    )
